# -*- coding: utf-8 -*-
"""Analysing_the_S&P500_Index.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1msOfw8uo3f8bbYPtgthWAyVHtSD20uJb
...
def sector_performance_visual(i):
  ...
  if i == "Communication Services":
    plot_df = pd.concat([plot_df.head(5), plot_df.tail(2)])
  else:
...
def sector_profitability_visual(i):
  ...
  plot_df = plot_df.reset_index()
  plot_df = plot_df.set_index("Ticker")
  plot_df['Net Profit Margin (TTM)'] = plot_df['Net Profit Margin (TTM)']
  plot_df = plot_df.sort_values(by="Net Profit Margin (TTM)", ascending=False)
...
for i, val in enumerate(plot_df['EPS']):
  if val > 0:
    ax.text(i, val + 2.5, str(round(val, 2)) + '$', ha='center', va='top', fontsize=12)
  else:
    ax.text(i, val - 2.5, str(round(val, 2)) + '$', ha='center', va='bottom', fontsize=12)
